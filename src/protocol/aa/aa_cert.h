#ifndef SRC_PROTOCOL_AA_AA_CERT
#define SRC_PROTOCOL_AA_AA_CERT

// Standard Android Auto head-unit client certificate, signed by the
// "Google Automotive Link" CA. This is the canonical open-source AA HU
// certificate first published by aasdk (f1x.studio) and still shipped by
// LIVI (github.com/f-io/LIVI) in 2026; phones accept it during the TLS
// handshake. If Google ever revokes it the handshake will complete but the
// phone will drop the link before service discovery.

static const char AA_HU_CERT_PEM[] = R"(-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----)";

static const char AA_HU_KEY_PEM[] = R"(-----BEGIN RSA PRIVATE KEY-----

KAwp3tIHPoJOQiKNQ3/qks5km/9dujUGU2ARiU3qmxLMdgegFz8e
-----END RSA PRIVATE KEY-----)";

#endif /* SRC_PROTOCOL_AA_AA_CERT */
